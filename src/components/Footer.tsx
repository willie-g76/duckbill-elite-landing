        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 mb-12">
          ...
          <div className="lg:col-span-1">
            <Link to="/" className="flex items-center gap-3 mb-6">
              <img src={logoFull} alt="Duckbill Roofing" className="h-16 w-16 object-contain rounded-full bg-primary-foreground/10" />
            </Link>
          ...
          <div>
            ...
            <ul className="space-y-3">
              {services.map(service => <li key={service.name}>
                  <Link to={service.href} className="text-sm text-primary-foreground/80 hover:text-accent transition-colors">
                    {service.name}
                  </Link>
                </li>)}
            </ul>
